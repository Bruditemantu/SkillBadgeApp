...
const AllBadges = () => {
    ...
    const [userData, setUserData] = useState([]); // Initialize as an empty array
          ...
          const fetchUserData = async () => {
            try {
          const response = await Axios.get("http://127.0.0.1:8000/api/org/crud/", {
            headers: {
      ...
      const handleDeleteBadge = async (badgeId) => {
        try {
          await Axios.delete(`http://127.0.0.1:8000/api/org/crud/?badge_id=${badgeId}`, {
            headers: {
              Authorization: `token ${localStorage.getItem('token')}`,
            },
          });
          // After successful delete, you may want to refresh the badge data
          fetchUserData();
        } catch (error) {
